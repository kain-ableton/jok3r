...
class Settings:
        ...
        for opt in optparsed:
            ...
            if opt == 'name':
                ...
                if tool_config[opt].startswith('python'):
                    m = re.match(
                        r'python(?P<version>[0-9](\.[0-9])*)', tool_config[opt])
                    if not m:
                ...
                if tool_config[opt].startswith('ruby'):
                    m1 = re.match(
                        r'ruby(?P<version>[0-9](\.[0-9])*)', tool_config[opt])
                    m2 = re.match(
                        r'ruby-(?P<version>[0-9](\.[0-9])*)', tool_config[opt])
                    if m1:
                ...
                tool_config[opt] = Command(
                    cmdtype=CmdType.INSTALL, cmdline=val)
    ...
    def __parse_section_specific_options(self, service, service_config):
        ...
        for opt in optparsed:
            ...
            opt_clean = StringUtils.clean(
                opt.lower(), allowed_specials=('-', '_'))
    ...
    def __parse_check_section(self, service, section, check_config):
        ...
        for opt in optparsed:
            ...
            if opt == 'name':
                check_config[opt] = StringUtils.clean(
                    val, allowed_specials=('_', '-'))
    ...
    def save(self, conf_filename):
        ...
        """
        try:
            config_file = FileUtils.concat_path(
                SETTINGS_DIR, conf_filename+CONF_EXT)
