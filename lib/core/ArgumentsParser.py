...
class ArgumentsParser:
    ...
    def toolbox(self):
        ...
        parameter = parser.add_argument_group(
            Output.colored('Parameter', attrs='bold'))
        ...
        check = parser.add_argument_group(
            Output.colored('Check', attrs='bold'))
        check.add_argument(
            '--check',
    ...
    def attack(self):
        ...
        multitargets = parser.add_argument_group(
            Output.colored(
                'Mode: Multiple targets from a mission scope', attrs='bold'),
    ...
    def __check_args_attack_selection(self):
        ...
        # Attack profile
            ...
            profile = self.settings.attack_profiles.get(
                self.args.profile.lower())
            if not profile:
        ...
        for product in self.args.products:
            if '=' in product:
                ...
                if not self.settings.services.is_product_type_supported(
                    ...
                    service = self.settings.services.get_service_for_product_type(
                        type_)
                ...
                logger.error(
                    'Invalid syntax for --product. Must be: <type=name>')
                return False
    ...
    def __check_args_attack_context_option(self):
        ...
        for option in self.args.options:
            if '=' in option:
                ...
                logger.error(
                    'Invalid syntax for --option. Must be: <name=value>')
                return False
